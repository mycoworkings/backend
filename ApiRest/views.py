  ...
  "coworkings": [
    ...
    if request.method == 'POST':
        ...
        try:
          ...
          location_to_save = location.objects.create(provinceCode=location_test["provinceCode"],munipalityCode=location_test["munipalityCode"],streetCode=location_test["streetCode"],postalCode=location_test["postalCode"],latitude=location_test["latitude"],longitude=location_test["longitude"])
...
@api_view(['POST','GET'])
@csrf_exempt
def GetCoworking(request, token):
  
  Final_response = GetCoworking_function(token)
...
def AllCoworkings (request):

  Array_coworkings = []

  Coworking_arrray = Coworking.objects.all()

  Coworking_arrray.count()

  for cow in range(1,Coworking_arrray.count() +1):

    Final_response = GetCoworking_function(cow)

    Array_coworkings.append(Final_response)
